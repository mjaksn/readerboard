...
def memory_claimed(allocations: list[FileAllocation]) -> int:
    ...
    What counts as a file's own size is not its size field for every file type,
    which is why this reads :attr:`FileAllocation.pool_bytes` rather than
    ``capacity``: a picture's size field is a geometry, so adding it to a budget
    charges a seven by sixteen icon 1808 bytes for something measured taking 56.
    """
    return memory_claimed_by(entry.pool_bytes for entry in allocations)
...
def memory_claimed_by(sizes: Iterable[int]) -> int:
    ...
    a pool would cost before there is a layout to build it from. A picture's
    size here is its ``pool_bytes`` rather than its geometry.
    """
    return (
